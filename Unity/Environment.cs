...
namespace indoorMobility.Scripts.Game
{
    public class Environment : MonoBehaviour {
        ...
        //public float distancetoTarget=player.finaldistancetotarget;
        ...
        private int _action;
        private byte[] _data;
        private byte _end, _reward, _distancetoTarget;
        //private byte[] _distancetoTarget;
        // Can be accessed by player script or game manager
        public byte Reward { set => _reward = value;}
        public byte End { set => _end = value;}
        //public byte[] DistancetoTarget { set => _distancetoTarget = value;}
        public byte DistancetoTarget { set => _distancetoTarget = value;}
        ...
        public byte[] Output { //output to python: 1 byte to determine if the loop ended, 1 byte for the reward and 16 x w x h bytes with the camera view of the agent (state)
            get {
                _data[0] = _end;
                _data[1] = _reward;
                // _data[2] = _distancetoTarget[0]; 
                // _data[3]  = _distancetoTarget[1];
                // _data[4]  = _distancetoTarget[2];
                // _data[5]  = _distancetoTarget[3];
                _data[2]= _distancetoTarget;
                ...
                for (var y = 0;
                    ...
                    for (var x = 0;
                        ...
                        _data[i + 3]  = colors[j].r;
                        _data[i + 4]  = colors[j].g;
                        _data[i + 5]  = colors[j].b;
                        _data[i + 6]  = objseg[j].r;
                        _data[i + 7]  = objseg[j].g;
                        _data[i + 8]  = objseg[j].b;
                        _data[i + 9]  = semseg[j].r;
                        _data[i + 10]  = semseg[j].g;
                        _data[i + 11] = semseg[j].b;
                        _data[i + 12] = normals[j].r;
                        _data[i + 13] = normals[j].g;
                        _data[i + 14] = normals[j].b;
                        _data[i + 15] = flow[j].r;
                        _data[i + 16] = flow[j].g;
                        _data[i + 17] = flow[j].b;
                        _data[i + 18] = depth[j].r;
        ...
        private void Start() {
            ...
            houseModel = HouseModel.GetComponent<HouseModel>();
            ...
            // Output data
            _data = new byte[3 + 16 * appData.Width * appData.Height];
        }

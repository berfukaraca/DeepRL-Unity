...
namespace indoorMobility.Scripts.Game
{
    public class GameManager : MonoBehaviour
    {
        ...
        Vector3 agent_pos= new Vector3(0f,0f,0f);
            ...
            while (true)
                ...
                {   case Command.None:
                    ...
                    case Command.Step:
                        ...
                        yield return new WaitForFixedUpdate();
                        Time.timeScale = 0;
                        agent=Player.GetComponent<Player>();
                        if (agent.error_bump==1){
                            agent.transform.position=agent.agent_position1;
                            agent.m_Rigidbody.position=agent.agent_position1;
                            Debug.Log("prev position= transform position,rb" + agent.transform.position + agent.m_Rigidbody.position);
                            Debug.Log("position changed back");
                        }
